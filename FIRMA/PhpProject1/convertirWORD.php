...
$queEmp = "
select  (select t.APELLIDOS_TRABAJADOR + ' ' + t.NOMBRES_TRABAJADOR from db_general.jcardenas.h_trabajador t WHERE t.CODIGO_TRABAJADOR=f.CODIGO_TRABAJADOR) as nombre_firmante,
...
// en windows
while ($rowEmp = sqlsrv_fetch_array($resEmp, SQLSRV_FETCH_ASSOC)) {
...
switch ($tipo_doc) {
    ...
    case "DICTAMEN":
        ...
        $xmlWordMege .= '<FECHA_DOC></FECHA_DOC>';
    ...
    case "INFORME":
        ...
        $xmlWordMege .= ' <TIPO_DOC>' . $tipo_doc . '</TIPO_DOC>';
        $xmlWordMege .= '<USUARIO>' . $usuario . '</USUARIO>';
        $xmlWordMege .= ' <EMPLEADO_REBIBE>'.$nom_destino.'</EMPLEADO_REBIBE>';
        $xmlWordMege .= ' <COSTO_RECIBE>'.$cargo.'</COSTO_RECIBE>';
...
if ($xmlWordMege != "") $xmlWordMege = base64_encode($xmlWordMege);
